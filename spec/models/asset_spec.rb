...
describe Asset do
  ...
  describe '#url' do
    ...
    context 'on s3 storage url has to be generated by expiring_url method' do
      before { described_class.stub(s3_storage?: true) }

      it 'generates expire url' do
        subject.data.should_receive(:expiring_url).with(3600, nil).and_call_original
        perform
      end
    end
  end
