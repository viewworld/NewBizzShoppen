...
describe NewsletterSynch do
  ...
  it "should export all new subscribers in sources" do
    ...
    CreateSend::Subscriber.stubs(:add).with(@list_lead_category.cm_list_id, @lead.email_address, @lead.contact_name, [], true)

    @contact = Contact.make!(:campaign => @campaign)
    CreateSend::Subscriber.stubs(:add).with(@list_campaign.cm_list_id, @contact.email_address, @contact.contact_name, [], true)

    @member = User::Member.make!
    CreateSend::Subscriber.stubs(:add).with(@list_role.cm_list_id, @member.email, @member.full_name, [], true)
