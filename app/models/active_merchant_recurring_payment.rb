class ActiveMerchantRecurringPayment
  ...
  def make_payment!
    ...
    @result = @gateway.purchase((@subperiod.billing_price * 100).to_i, @subperiod.subscription.payment_profile_id, :order_id => "ssp#{@subperiod.subscription.id}_#{@subperiod.id}")

    if @result.success?
      @subperiod.update_attributes(:payment_paid_auto => true, :payment_retry_at => nil)
      if @subperiod.subscription.subscription_sub_periods.last == @subperiod
