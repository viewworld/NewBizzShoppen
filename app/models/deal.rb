class Deal < AbstractLead
  ...
  def build_supplier(params={})
    if supplier
      ...
      user = User::CategorySupplier.new({:email => email_address, :company_name => company_name.strip, :phone => phone_number,
                                 ...
                                 :assign_free_subscription_plan => true, :dont_show_my_deals => false}.merge(params))
  ...
  def self.group_deals_for_select
    group_deals.without_inactive.map { |gd| [gd.to_s_for_group_deals_for_select, gd.id] }.sort
  end

  def self.all_deals_for_select
    without_inactive.published_only.map { |gd| [gd.to_s_for_group_deals_for_select, gd.id] }.sort
  end
