class Category < ActiveRecord::Base
  ...
  scope :without_locked, with_translations.where("is_locked = ?", false).order('category_translations.name').select('categories.*, category_translations.name')
  scope :with_leads, with_translations.select('categories.*, category_translations.name').where("total_leads_count > 0").order('category_translations.name')
