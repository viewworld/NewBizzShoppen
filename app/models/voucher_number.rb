class VoucherNumber < ActiveRecord::Base
  ...
  def activate!(payment_notification)
    ...
    transaction_klass = payment_notification.is_a?(PaypalVoucherPaymentNotification) ? PaypalTransaction : ActiveMerchantTransaction
    transaction_klass.create(:invoice => invoice, :payment_notification => payment_notification, :amount => deal.discounted_price, :paid_at => Time.now)
