class Subscription < ActiveRecord::Base
  ...
  def self.canceled_in_payment_gateway(prof_id, spn)
    if sub_period = SubscriptionSubPeriod.payment_unpaid.for_paypal.for_recurring_payment(prof_id).readonly(false).first
      ...
      if !subscription.admin_changed? and !subscription.upgraded?
  ...
  def apply_time_constraints(_start_date)
    ...
    if subscription_period > 0
      ...
      if free_period_can_be_applied?
        ...
        CompanyVat.create(:vat_number => user.vat_number.strip) unless payment_billing_at_start? and unconfirmed_payment?
      end
  ...
  def handle_user_privileges
    ...
    if newsletter_manager?
      _user = user.with_role
      _user.send(:cm_synchronize!)
      _user.send(:cm_set_access!)
      _user.setup_empty_list!
    end
  end
