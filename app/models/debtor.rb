class Debtor < ActiveRecord::Base
  ...
  scope :with_keyword, lambda { |keyword| where("owner_id in (select owner_id from debtors where lower(header) like :keyword) OR lower(contact_name) like :keyword OR lower(company_name) like :keyword", :keyword => "%#{keyword.to_s.downcase}%") }
