class LeadPurchase < LeadPurchaseBase
  ...
  def deliver_bought_notification
    if accessible_from_changed? and !accessible_from.nil? and !owner.disable_bought_notification
      if lead.category.email_template.blank?
        ...
        TemplateMailer.delay.new(owner.email, lead.category.email_template, Country.get_country_from_locale, {:lead_purchase => self, :sender_id => nil})
      end
    end
  end
  ...
  def deliver_email_template(uniq_id, to=nil)
    TemplateMailer.delay.new(to.blank? ? owner.email : to, uniq_id.to_sym, Country.get_country_from_locale, {:lead_purchase => self, :sender_id => nil})
  end
