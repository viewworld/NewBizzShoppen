class LeadCertificationRequest < ActiveRecord::Base
  ...
  def process
    ...
    self.save!
    TemplateMailer.new(contact_email, :certification_request, Country.get_country_from_locale, {:lead_certification_request => self, :sender_id => nil}).deliver! unless do_not_send_email
  end
  ...
  def update_state!
    if [STATE_SENT, STATE_SENT_REMINDER].include?(state) and last_email_sent_at.to_date+Settings.resend_certification_notification_after_days.to_i == Date.today
      ...
      self.last_email_sent_at = Time.now
      self.save
      TemplateMailer.new(contact_email, :certification_request_reminder, Country.get_country_from_locale, {:lead_certification_request => self, :sender_id => nil}).deliver!
