class AbstractLead < ActiveRecord::Base
  ...
  scope :with_keyword, lambda { |q| where("lower(header) like :keyword OR lower(leads.description) like :keyword OR lower(creator_name) like :keyword OR lower(company_name) like :keyword", {:keyword => "%#{q.downcase}%"}) }
