class PaymentNotification < ActiveRecord::Base
  ...
  def generate_invoice
    if status == "Completed" and params[:receiver_email] == APP_CONFIG[:paypal_email] and params[:secret] == APP_CONFIG[:paypal_secret] and
      ...
      invoice = Invoice.create(:user_id => buyer.id, :paid_at => self.created_at)
      PaymentTransaction.create(:invoice => invoice, :payment_notification => self, :amount => buyer.cart.total, :paid_at => self.created_at)
