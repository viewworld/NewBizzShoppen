module CampaignsHelper
  ...
  def campaigns_table(collection, options = {}, &block)
    ...
    render(:partial => '/campaigns/table', :locals => options.merge({:collection => collection}.merge(@hb.results)))
  end
  ...
  def contacts_table(collection, options = {}, &block)
    ...
    render(:partial => '/contacts/table', :locals => options.merge({:collection => collection}.merge(@hb.results)))
  end
  ...
  def call_results_table(collection, options = {}, &block)
    ...
    options[:show_checkboxes] ||= false
    options[:agent_work_screen] ||= false
    render(:partial => '/call_results/table', :locals => options.merge({:collection => collection}.merge(@hb.results)))
  end
