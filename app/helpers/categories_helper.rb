...
=end
  ...
  def all_categories_tree(options={})
    ...
    content_tag(:ul, root_categories.map { |c| content_tag(:li, (options[:roots] ? category_label(c, options).html_safe : category_tree(c, options)), :class => "categories_node", :id => dom_id(c)) }.join.html_safe, :class => "categories_tree vertical", :id => "categories_main_tree")
  end
