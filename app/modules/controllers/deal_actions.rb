module DealActions
  ...
  def index
    ...
    @deals = current_user.admin? ? @search.paginate(:page => params[:page], :per_page => Settings.default_deals_per_page, :show_all => params[:show_all] == "1") :
        @search.where("creator_id IN (?) or email_address = ? or deal_admin_email = ?", (current_user.call_centre? ? current_user.subaccounts.map(&:id) + [current_user.id] : [current_user.id]), current_user.email, current_user.email).paginate(:page => params[:page], :per_page => Settings.default_leads_per_page, :show_all => params[:show_all] == "1")
  end
