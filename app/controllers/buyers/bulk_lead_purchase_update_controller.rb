  ...
  def create
    if params[:lead_purchase_ids]
      current_user.lead_purchases.where(:id => params[:lead_purchase_ids]).each do |lead_purchase|
        ...
        lead_purchase.rating_level = params[:bulk_rating_level] if params[:bulk_rating_level]
        lead_purchase.rating_reason = params[:bulk_rating_reason] if params[:bulk_rating_reason]
        lead_purchase.save
      end
