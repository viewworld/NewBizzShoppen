class AgentHomeController < ApplicationController
  ...
  def guest
    ...
    if user_signed_in?
      @latest_leads = current_user.has_any_role?(:agent, :call_centre, :call_centre_agent, :purchase_manager) ? @latest_leads.with_agent_unique_categories(current_user.id) : current_user.has_accessible_categories? ? @best_sellers.within_accessible_categories(current_user.accessible_categories_ids) : @best_sellers.with_supplier_unique_categories(current_user.id)
    else
