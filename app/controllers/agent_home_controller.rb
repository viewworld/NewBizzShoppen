class AgentHomeController < ApplicationController
  ...
  def show
    ...
    @best_sellers = @best_sellers.published_only.without_bought_and_requested_by(current_user).bestsellers
    if user_signed_in?
    ...
    @latest_leads = @latest_leads.published_only.without_bought_and_requested_by(current_user)
    if user_signed_in?
      @latest_leads = current_user.has_any_role?(:agent, :call_centre_agent, :purchase_manager) ? @latest_leads.with_agent_unique_categories(current_user.id) : current_user.has_accessible_categories? ? @best_sellers.within_accessible_categories(current_user.accessible_categories_ids) : @best_sellers.with_customer_unique_categories(current_user.id)
    else
