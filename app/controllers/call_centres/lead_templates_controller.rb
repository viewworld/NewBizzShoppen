class CallCentres::LeadTemplatesController < CallCentres::CallCentreController
  ...
  def create
    ...
    create! do |success, failure|
      success.html { redirect_to call_centres_lead_templates_path }
  ...
  def destroy
    ...
    if @lead_template.destroy
      flash[:notice] = I18n.t("call_centre.lead_templates.destroy.flash.deletion_successful")
    else
      flash[:notice] = I18n.t("call_centre.lead_templates.destroy.flash.deletion_failure")
    end
    redirect_to :back
  end
  ...
  def collection
    ...
    @categories = LeadCategory.without_locked.with_call_centre_unique(current_user).map { |c| [c.name.to_s, c.id] }.sort
  end
end