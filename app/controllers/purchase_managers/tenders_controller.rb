class PurchaseManagers::TendersController < PurchaseManagers::PurchaseManagerController
  ...
  def destroy
    @lead = current_user.leads.find(params[:id])
    if @lead.destroy
      flash[:notice] = I18n.t("purchase_manager.tenders.destroy.flash.tender_deletion_successful")
    else
      flash[:notice] = I18n.t("purchase_manager.tenders.destroy.flash.tender_deletion_failure")
    end
    redirect_to purchase_managers_tenders_path
  end
