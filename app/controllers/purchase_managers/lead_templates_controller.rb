class PurchaseManagers::LeadTemplatesController < PurchaseManagers::PurchaseManagerController
  inherit_resources
  ...
  def destroy
    ...
    if @lead_template.destroy
      flash[:notice] = I18n.t("purchase_manager.lead_templates.destroy.flash.deletion_successful")
    else
      flash[:notice] = I18n.t("purchase_manager.lead_templates.destroy.flash.deletion_failure")
    end
    redirect_to :back
  end
  ...
  def collection
    ...
    @categories = Category.without_locked.with_agent_unique(current_user).map { |c| [c.name, c.id] }
  end
end