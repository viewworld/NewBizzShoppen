class BuyerHomeController < ApplicationController
  ...
  def sales_manager
    @latest_leads = Lead.without_inactive.interesting_for_user(current_user).published_only.without_bought_and_requested_by(current_user).with_customer_unique_categories(current_user.id).latest.limit(3)
