class LeadUsers::LeadRequestsController < LeadUsers::LeadUserController
  ...
  def collection
    ...
    @countries = Country.with_lead_request_requested_by(current_user).map{|c| [c.name.to_s, c.id]}.sort
    @categories = LeadCategory.with_lead_request_requested_by(current_user).map{|c| [c.name.to_s, c.id]}.sort
