class ContactsAdvancedImportsController < SecuredController
  ...
  def create
    ...
    if Sheet.validate_attachment(@attachment_file)
      result = @model.advanced_import_contacts_from_xls(Sheet.new(@attachment_file, true).roo_instance, params[:object_field], params[:spreadsheet_field], current_user, @object_id)
