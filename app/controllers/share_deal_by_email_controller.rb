class ShareDealByEmailController < ApplicationController
  ...
  def create
    ...
    @email_preview = ShareDealTemplatePreview.new('share_deal_by_email', params[:email_from], {:description => params[:body], :name => params[:name], :deal_url => deal_url(:id => @deal.slug)})
