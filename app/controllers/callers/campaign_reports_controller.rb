  ...
  def collection
    ...
    if @views_count > 0
      if @per_user
        ...
        @campaign_reports = @campaigns.map { |campaign| CampaignReport.new(campaign, @date_from, @date_to, (current_user.has_role?(:admin) and @selected_agents) ? @selected_agents : nil, @result_ids) }
      end
    else
      @campaign_reports = []
      @campaign_users = []
