...
#        end
      ...
      def set_has_one_or_belongs_to_association(original_object, association, options)
        if sub_kopy = (options[:associations] || {}).keys.include?(original_object.send(association)) ? options[:associations][original_object.send(association)] : original_object.send(association)
          ...
          sub_kopy.save_without_callbacks = true if !options[:with_callbacks]
          sub_kopy
        end
      end
      ...
      def set_has_and_belongs_to_many_association(original_object, association, options)
        original_object.send(association).collect do |object|
          object.save_without_callbacks = true if !options[:with_callbacks]
          (options[:replace_object_associations_only] and options[:associations].keys.include?(object)) ? options[:associations][object] : object
        end
      end

      def save_in_cloning(options)
        self.save_without_callbacks = true if !options[:with_callbacks]
