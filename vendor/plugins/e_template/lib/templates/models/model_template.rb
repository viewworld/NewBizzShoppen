class EmailTemplate < ActiveRecord::Base
  ...
  def self.get_hash_with_available_objects_and_methods
    r = {}
    Dir.glob("app/models/**/*.rb").map{|path| path.sub('app/models/','').split('/').map{|p| File.basename(p).gsub('.rb','').pluralize.classify}*'::'}.each do |model_name|
      if (model = model_name.constantize).instance_methods.include?('to_liquid')
        r[model.to_s.underscore.to_sym] = model::LIQUID_METHODS.call(self).keys if model.constants.include?('LIQUID_METHODS')
      end
    end
    r
  end
