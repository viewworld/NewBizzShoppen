...
function remove_fields(t)
{
    ...
    {        
        $(t).prev("input[name$='[_destroy]']").val("1");
        recalcValues();
    });
...
function add_fields(link, association, content)
{
    ...
    $(link).parents('tr.total').before(content.replace(regexp, new_id));

    //$(link).parents("tr.total").find("input[name$='[_destroy]']").parents(".invoice_line:last").after(content.replace(regexp, new_id));
    //$(link).parents("form").find("input[name$='[_destroy]']").parents(".invoice_line:last").hide();
    //$(link).parents("form").find("input[name$='[_destroy]']").parents(".invoice_line:last").slideFadeToggle();
