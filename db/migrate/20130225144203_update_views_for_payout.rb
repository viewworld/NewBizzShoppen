class UpdateViewsForPayout < ActiveRecord::Migration
  def self.up
    execute %{
      CREATE OR REPLACE VIEW agent_timesheets_cost AS
         SELECT user_session_logs.user_id, user_session_logs.campaign_id, user_session_logs.end_date AS created_at,
                ...
                CASE
                    ...
                    WHEN campaigns.cost_type = 4 THEN (SELECT sum(euro_payout) FROM agent_timesheets_payouts WHERE user_id = user_session_logs.user_id AND created_at = user_session_logs.end_date)
