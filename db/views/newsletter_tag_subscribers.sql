...
FROM (((((((newsletter_sources
           ...
           JOIN taggings tag_group ON (((tag_group.taggable_id = tag_groups.id)
                                        AND ((tag_group.taggable_type)::text = 'TagGroup'::text))))
        ...
        LEFT JOIN addresses ON (((addresses.addressable_id = users.id)
                                 AND ((addresses.addressable_type)::text = (taggings.taggable_type)::text))))
       LEFT JOIN campaigns ON ((campaigns.id = taggings.taggable_id)))
      LEFT JOIN leads ON ((leads.id = taggings.taggable_id)))
WHERE (((((taggings.taggable_type)::text <> 'TagGroup'::text)
         AND ((taggings.taggable_type)::text <> 'SurveyOption'::text))
        AND (newsletter_sources.source_type = 4))
       AND (tag_groups.match_all IS FALSE))
GROUP BY newsletter_sources.id,
         newsletter_sources.newsletter_list_id,
...
WHERE (((((taggings.taggable_type)::text <> 'TagGroup'::text)
         AND ((taggings.taggable_type)::text <> 'SurveyOption'::text))
        AND (newsletter_sources.source_type = 4))
       AND (tag_groups.match_all IS TRUE))
GROUP BY newsletter_sources.id,
         newsletter_sources.newsletter_list_id,
         newsletter_sources.sourceable_id,
         taggings.taggable_type,
         taggings.taggable_id,
         users.email,
         leads.email_address,
         ...
         addresses.zip_code,
         leads.zip_code,
         users.login_key HAVING (count(*) =
                                   (SELECT count(*) AS COUNT
                                    FROM taggings
                                    WHERE (((taggings.taggable_type)::text = 'TagGroup'::text)
                                           AND (taggings.taggable_id = newsletter_sources.sourceable_id))));
