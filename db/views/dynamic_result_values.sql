...
WHERE result_values.field_type::text ~~ '1'::text AND campaigns_result_fields.is_dynamic_value = true AND campaigns_results.is_dynamic_value = true AND contacts.type::text = 'Contact'::text AND results.upgrades_to_lead IS FALSE AND campaigns_results.is_reported IS TRUE AND results.final IS TRUE;
