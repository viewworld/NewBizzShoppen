...
GROUP BY user_session_logs.user_id,
         user_session_logs.end_date,
         date_part('dow'::text, user_session_logs.end_time),
         date_part('week'::text, user_session_logs.end_time),
         user_session_logs.campaign_id HAVING (sum(user_session_logs.hours_count) > (0)::double PRECISION);
