...
FROM ((call_results
      ...
      JOIN campaigns_results ON (((campaigns_results.campaign_id = contacts.campaign_id)
                                  AND (campaigns_results.result_id = call_results.result_id))))
WHERE (campaigns_results.is_reported IS TRUE)
GROUP BY call_results.creator_id,
         (call_results.created_at)::date,
         contacts.campaign_id HAVING (sum(call_results.euro_payout) > (0)::numeric);
