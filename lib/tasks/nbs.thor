...
{% endfor %}"}
        ...
        {:name => 'survey_recipients',
         :uniq_id => 'survey_recipients',
         :en => {:subject => 'Survey recipients export file is ready',
                 :body => "<p>Welcome {{user.email}}!</p><p>Survey recipients export file you've requested is now ready. To view it, follow this link:</p><p><a href=\"{{timesheet_url}}\">Survey recipients</a></p>"},
         :da => {:subject => '[DK] Survey recipients export file is ready',
                 :body => "<p>Welcome {{user.email}}!</p><p>Survey recipients export file you've requested is now ready. To view iŃt, follow this link:</p><p><a href=\"{{timesheet_url}}\">Survey recipients</a></p>"},
        },
        {:name => 'newsletter_subscriber_sources_tagging',
         :uniq_id => 'newsletter_subscriber_sources_tagging',
         :en => {:subject => 'Newsletter list subscriber sources tagging has been completed',
                 :body => '<p>Welcome {{user.email}}!</p><p>Newsletter list subscriber sources tagging for tags: [{{tags}}], has been completed.</p>'},
         :da => {:subject => '[DK] Newsletter list subscriber sources tagging has been completed',
                 :body => '<p>Welcome {{user.email}}!</p><p>Newsletter list subscriber sources tagging for tags: [{{tags}}], has been completed.</p>'},
        },
  ...
  def import_contacts_from_newsletter_lists
    Campaign.where(:import_contacts_from_lists_enabled => true).each do |campaign|
      campaign.import_contacts_from_newsletter_lists!
    end
  end
end
