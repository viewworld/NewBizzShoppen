...
Feature: Subscription plan setup
  ...
  Background:
    ...
    Given subscription plan exists with attributes "name:XPayable with paypal,use_online_payment:1"
    And subscription plan has following lines
      | name                 | price |
      | subscr premium line1 |    99 |
    Given subscription plan exists with attributes "name:NOT Payable with paypal,use_online_payment:0"
    And subscription plan has following lines
      | name                 | price |
      | subscr premium line1 |    22 |
    Given subscription plan exists with attributes "name:Member NOT Payable with paypal,use_online_payment:0,assigned_roles:member"
    And subscription plan has following lines
      | name                 | price |
      | subscr premium line1 |    22 |
  ...
  Scenario: I can enable subscription to be handled by Paypal
    When I check "subscription_plan_use_online_payment"
  ...
  Scenario: I can enable automatic downgrading
    ...
    And I fill in "subscription_plan_payment_retries" with "2"
  ...
  Scenario: The must enter billing and end of free period can only be enabled when there is free period for subscription plan
    ...
    And I choose "subscription_plan_payment_billing_at_start_false"
