Given /^paypal payment for user with email "([^"]*)" and role "([^"]*)"$/ do |email, role|
  ...
  rack_test_session_wrapper.process(:post, "/payment_notifications", :txn_id => "irek", :payment_status => "Completed", :secret => APP_CONFIG[:paypal_secret], :receiver_email => APP_CONFIG[:paypal_email], :mc_gross => BigDecimal(customer.cart.total.to_s).to_s, :invoice => customer.cart.id)
end
