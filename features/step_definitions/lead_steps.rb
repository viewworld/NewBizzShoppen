...
Given /^lead "([^"]*)" should have field "([^"]*)" with value "([^"]*)"$/ do |header, field, value|
  Lead.where(:header => header).order("created_at DESC").first.send(field.to_sym).to_s.should == value.to_s
end