...
Feature: General deals
  ...
  # @_done @_tested @tgn @selenium @_deprecated2014
  ...
  # @_done @_tested @selenium @tgn @auto_buy_max @_deprecated2014
...
#    And category named "Xeper" is auto buy enabled
  ...
  #7448
  # @m20 @_requested @selenium @tgn @_tested @_done @_deprecated2014
  ...
  #7847
  # @m20 @_requested @tgn @_done @_tested @_deprecated2014
  ...
  # @m21 @_requested @is @selenium @_done @_tested @_deprecated2014
  ...
  # #8328 #14071
  # @m22 @_requested @selenium @_done @_tested @ao @_deprecated
  # Scenario: I can enter "Featured description" for each deal (rich text + images)

  #8328
  @m22 @_requested @selenium @_done @_tested @ao
  Scenario: I can enter "Short featured description" for each deal (rich text)
    Given I am not sign in
    And I make sure current locale is "da"
    Given user buyer@nbs.com with role supplier exists with attributes "company_name:Xeper"
    And user "buyer@nbs.com" has deal maker role enabled
    ...
    And I click hidden link by url regex "/suppliers\/deals\/\d+\/edit/"
    And I wait 1 second
    Then I should see CSS path "textarea#deal_short_featured_description_editor"
  ...
  # #8329
  # @m22 @_requested @selenium @non_testable @_done @ao
  # Scenario: I can upload images in every rich text editor

  # @m24 @_requested @auto_buy_max @_done @_tested_elsewhere @tgn
